...
<div class="panel panel-flat">
	<?php
		if (isset($errorMessage)) {						
			...
			echo '	<button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Close</span></button>';
			echo '	<span class="text-semibold">'.$successMessage;
			echo '</div>';
		}
	?>
	<!-- DataTables -->
	...
	$(document).ready(function() {
		...
		$("body").delegate(".download","click",function(){
				...
				$.ajax({
					url: 'getHtml',
					type: 'POST',
					data: "source=payslip&id="+id,
					success:function(response){
						/*$.ajax({
							url: 'getPDF',
							type: 'POST',
							data: "id="+id+"&content="+response,
...
function showDeleteConfirm(id){
	swal({
			title: "Are you sure want to delete this Payslip?",
			...
			if (isConfirm) {
				$.ajax({
					...
					success:function(response){
						...
						function(isConfirm){
							if(isConfirm){
								window.location.href = "<?php echo site_url('../POC/list_payslip/'); ?>"
							}
						});
					}
				})
